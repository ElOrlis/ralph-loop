  ...
  const project = {
    ...
    fieldIds: {
      ...
      ralphStatus:      { id: 'S1', dataType: 'SINGLE_SELECT',
        options: { Pending: 'op', 'In Progress': 'oi', Passed: 'opa', Failed: 'of', Stalled: 'ost', Blocked: 'ob' } },
    },
  };
  const task = {
...
test('sync-project-item sets ralphStatus=Blocked when task.status=blocked', () => {
  projects.updateItemField.mockReturnValue(undefined);
  getCallCount.mockReturnValue(5);
  const project = {
    id: 'PVT_test', number: 99,
  ...
  const task = { id: 't', priority: 1, category: 'X', attempts: 2, status: 'blocked', passes: false, projectItemId: 'PVTI_test' };
  const results = { results: [{ passed: false }] };
  const { stdout } = runCli([
    'sync-project-item',
    '--project', JSON.stringify(project),
    '--task', JSON.stringify(task),
  ...
  expect(JSON.parse(stdout).ralphStatus).toBe('Blocked');
});
