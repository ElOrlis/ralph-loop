  ...
  switch (command) {
    ...
    case 'update-issue': {
      ...
      if (!repo || !issueNumber || !iteration || !maxIterations || !resultsJson || !criteriaJson) {
        console.error('Usage: node lib/github/index.js update-issue --repo owner/name --issue N --iteration N --max-iterations N --results \'<json>\' --criteria \'<json>\' [--mcp-status <status>]');
        process.exit(1);
      }
      updateIssue({
        repo,
    ...
    default:
      ...
      console.error('Commands: resolve-repo, create-issue, update-issue, close-issue, add-label, remove-label, create-project, ensure-project-item, sync-project-item, validate-project, repair-project-fields, ensure-pr, mark-pr-ready');
      process.exit(1);
  }
}
