...
function prExistsForBranch({ repo, branchName }) {
  try {
    const out = execSync(
      `gh pr view ${quote(branchName)} --repo ${quote(repo)} --json number,url`,
      { encoding: 'utf-8' }
    );
...
function ensureDraftPR({ repo, branchName, baseBranch, taskId, taskTitle, issueNumber }) {
  ...
  const cmd = [
    ...
    `--repo ${quote(repo)}`,
    '--draft',
    `--base ${quote(baseBranch)}`,
    `--head ${quote(branchName)}`,
    `--title "${escapedTitle}"`,
    `--body-file "${bodyFile}"`,
  ].join(' ');
...
function markPRReady({ repo, prNumber }) {
  try {
    execSync(`gh pr ready ${prNumber} --repo ${quote(repo)}`, { encoding: 'utf-8' });
  } catch (err) {
