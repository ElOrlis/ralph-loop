...
function currentBranch() {
  try {
    return execSync('git rev-parse --abbrev-ref HEAD', { encoding: 'utf-8' }).toString().trim();
  } catch (err) {
...
function ensureBranch({ name, baseBranch }) {
  ...
  let baseSha;
  try {
    baseSha = execSync(`git rev-parse ${quote(baseBranch)}`, { encoding: 'utf-8' }).toString().trim();
  } catch (err) {
