  ...
  switch (command) {
    ...
    case 'commit-iteration': {
      ...
      if (!taskId || !ralphStatus || isNaN(iteration) || isNaN(maxIterations)) {
        usage('Usage: commit-iteration --task-id --task-title --iteration --max-iterations --pass-count --total-count --status [--issue N] [--agent <name>] [--skip-if-empty]');
      }
      const sha = commitIteration({
        taskId, taskTitle, iteration, maxIterations,
        passCount: passCount || 0, totalCount: totalCount || 0,
        issueNumber, ralphStatus, agent, skipIfEmpty,
      });
    ...
    default:
      ...
      console.error('Commands: slugify, branch-name, current-branch, ensure-branch, switch-to, has-uncommitted, stash-push, stash-pop, commit-iteration, push, merge-branch, merge-abort');
      process.exit(1);
  }
} catch (err) {
