...
function format(report) {
  ...
  // Section 5: Agent breakdown (omitted when null)
  if (report.agentBreakdown) {
    lines.push(`${BLUE}Agent breakdown${NC}`);
    lines.push('---------------');
    for (const [name, count] of Object.entries(report.agentBreakdown.iterations)) {
      lines.push(`  ${name}: ${count}`);
    }
    lines.push(`  Reviewer invocations: ${report.agentBreakdown.reviewerInvocations}`);
    for (const [name, count] of Object.entries(report.agentBreakdown.reviewerByAgent)) {
